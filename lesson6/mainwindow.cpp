...
void MainWindow::on_actionlang_triggered() {
  qTranslator.load(":/QtLanguage_by.qm");
  qApp->installTranslator(&qTranslator);
  ui->retranslateUi(this);
  // не понял как вернуть обратно только знаю как чтобы без доп словоря
}
...
void MainWindow::on_actionToPrint_triggered() {
  printDoc = qobject_cast<QPlainTextEdit *>(ui->plainTextEdit);
  QPrinter printer;
